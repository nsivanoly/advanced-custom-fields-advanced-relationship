...
if( ! class_exists('acf_field_advanced_relationship') ) :

    class acf_field_advanced_relationship extends acf_field {
        ...
        */
        ...
        function initialize() {
            ...
            // vars
            $this->name = 'advanced_relationship';
            $this->label = __("Advanced Relationship REST",'acf');
            ...
            // extra
            add_action('wp_ajax_acf/fields/advanced_relationship/query',			array($this, 'ajax_query'));
            add_action('wp_ajax_nopriv_acf/fields/advanced_relationship/query',	array($this, 'ajax_query'));
        ...
        */
        ...
        function get_ajax_query( $options = array() ) {
            ...
            // filters
            $args = apply_filters('acf/fields/advanced_relationship/query', $args, $field, $options['post_id']);
            $args = apply_filters('acf/fields/advanced_relationship/query/name=' . $field['name'], $args, $field, $options['post_id'] );
            $args = apply_filters('acf/fields/advanced_relationship/query/key=' . $field['key'], $args, $field, $options['post_id'] );
        ...
        */
        ...
        function get_post_title( $post, $field, $post_id = 0, $is_search = 0 ) {
            ...
            // filters
            $title = apply_filters('acf/fields/advanced_relationship/result', $title, $post, $field, $post_id);
            $title = apply_filters('acf/fields/advanced_relationship/result/name=' . $field['_name'], $title, $post, $field, $post_id);
            $title = apply_filters('acf/fields/advanced_relationship/result/key=' . $field['key'], $title, $post, $field, $post_id);
        ...
        */
        ...
        function render_field( $field ) {
            ...
            $atts = array(
                'id'				=> $field['id'],
                'class'				=> "acf-advanced_relationship {$field['class']}",
                'data-min'			=> $field['min'],
                'data-max'			=> $field['max'],
                'data-s'			=> '',
                'data-post_type'	=> '',
        ...
        function format_value( $value, $post_id, $field ) {
            ...
            if( $field['return_format'] == 'object' ) {
                ...
                foreach ($value as $item) {
                    $rst = new WP_REST_Posts_Controller($item->post_type);
                    $req = array("id" => $item->ID );
                    $data    = $rst->get_item(  $req, '' );
                    array_push($valData,$data->data);
                }
                $value = $valData;
            }
    ...
    // initialize
    new acf_field_advanced_relationship();
